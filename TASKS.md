...
**Atualização operacional de 2026-08-07 — estado atual:** o registro de 2026-08-06 acima permanece histórico, mas sua sessão e seu apontamento direto para o PROVISION foram superados por `AUTH-PROVISION-EXEC` bloqueado em `iamApiDisabled`, zero recurso criado, revogação nominal e estado zero. Depois do `IAM-API-ENABLEMENT-PREP`, um novo login isolado foi concluído e o `ADMIN-B2A5-INVENTORY-AUTH-IAM-API-ENABLEMENT-EXEC` terminou com classificação **A. IAM API HABILITADA E VALIDADA; IAM CREDENTIALS API HABILITADA COMO EFEITO DOCUMENTADO; CREDENCIAL ISOLADA ATIVA E GOVERNANÇA HABILITADA**. No projeto `turismo-sms`, lifecycle `ACTIVE` e fingerprint `68cf9cf1208055a962c614232e75b8a0b4f4f7564865e77e2a84382a87bd8c60`, `iam.googleapis.com` e `iamcredentials.googleapis.com` foram observadas inicialmente `DISABLED` e finalmente `ENABLED`; `serviceusage.services.enable` e `serviceusage.services.use` foram aprovadas. Somente `iam.googleapis.com` foi explicitamente solicitada, em uma execução com exit code `0`; `iamcredentials.googleapis.com` foi observada habilitada como efeito documentado. A credencial humana permanece ativa somente no diretório isolado, com uma conta credentialed e uma ativa correspondentes ao operador em memória, zero ADC, projeto persistido, impersonação e access-token file. Permanecem zero custom role, conta de serviço, binding e chave; Firestore/Datastore e Storage não foram acessados e o inventário não foi executado. O deadline de inatividade vigente no início desta governança era `2026-08-07T14:07:16.3697759Z`, e o absoluto permanece `2026-08-07T21:01:20.8006634Z`; pausa, bloqueio, impossibilidade de continuidade ou vencimento exigem revogação nominal.
...
**`ADMIN-B2A5-INVENTORY-AUTH-PROVISION-EXEC` — próximo bloco; não iniciado e dependente de gate e autorização próprios.**

- O novo login isolado, o `IAM-API-ENABLEMENT-EXEC` e esta governança sucedem cronologicamente a sessão de 2026-08-06, a tentativa bloqueada por `iamApiDisabled` e a revogação que comprovou estado zero. A credencial atual permanece ativa somente na configuração isolada, com janela temporal própria.
- O PROVISION exige preflight temporal, credencial ainda válida, confirmação de `iam.googleapis.com` e `iamcredentials.googleapis.com` habilitadas, todos os gates fail-closed já aprovados, parâmetros obrigatórios somente em memória e autorização humana literal própria. O ENABLEMENT e esta governança **não autorizam nem iniciam o PROVISION**.
- O bloco criará, somente após todos os gates fail-closed, o custom role e a conta de serviço aprovados; não cria binding, ADC, impersonação, chave, Firestore, Storage ou inventário.
- Sequência obrigatória: `AUTH-PROVISION-EXEC` → `PROVISION-GOVERNANCE` → `ACTIVATION-PREP` → `ACTIVATION-EXEC` → `INVENTORY-EXEC` → `AUTH-REVOKE`. Não apontar diretamente para ACTIVATION ou INVENTORY.
- Pausa, bloqueio, impossibilidade de continuidade ou vencimento exige revogação nominal imediata da credencial isolada.
