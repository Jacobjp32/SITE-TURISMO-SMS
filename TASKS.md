...
**`ADMIN-B2A5-INVENTORY-AUTH-CLI-SETUP-LOGIN-EXEC` (outra execução) — único próximo bloco possível; não iniciado e não autorizado.**

- **Não apontar para o `AUTH-PROVISION-EXEC`:** não existe credencial ativa. O ciclo anterior de login foi encerrado por revogação em 2026-08-05 e o estado atual é zero.
- **É uma nova execução, não uma repetição.** Exige novamente operador em memória, confirmação de prontidão da cadeia e **autorização humana literal própria**; a autorização anterior **não pode ser reutilizada**.
- **Gate operacional obrigatório antes de iniciar — agora com precedente concreto:** confirmar que `LOGIN-GOVERNANCE`, `AUTH-PROVISION-EXEC`, `PROVISION-GOVERNANCE`, `ACTIVATION-PREP`, `ACTIVATION-EXEC`, `INVENTORY-EXEC` e `AUTH-REVOKE` estão prontos para execução praticamente contínua. O ciclo de 2026-08-04 comprovou o risco: o login foi tecnicamente perfeito e ainda assim se perdeu por intervalo entre blocos.
- Baseline pré-login exigido: **zero contas credentialed**. Novos prazos de 60 minutos e 8 horas passarão a correr a partir da nova conclusão.
...
- **Não é necessária reinstalação:** instalação 578.0.0, Python empacotado 3.14.6 e diretório isolado permanecem disponíveis e íntegros.
- Histórico preservado: o `LOGIN-EXEC` havia sido habilitado pelo `EXECUTABLE-RECOVERY-PREP-DECISION` de 2026-08-04, que aprovou a **opção A** e elevou o parecer do PREP de **B** para **A. PRONTO PARA NOVO ADMIN-B2A5-INVENTORY-AUTH-CLI-SETUP-LOGIN-EXEC**, com prompt-ready integral produzido. O contrato corretivo de detecção então definido **funcionou na execução seguinte** e continua vinculante.
...
- Pré-requisito operacional: exige CLI instalada e operador autenticado, usando o caminho absoluto de `gcloud.cmd` e o `CLOUDSDK_CONFIG` isolado. A CLI já está instalada, o isolamento foi comprovado pelo `CLI-SETUP-ISOLATION-REPAIR-EXEC` e o `CLI-SETUP-LOGIN-PREP` foi elevado a **A** pelo `LOGIN-PREP-DECISION`, tudo em 2026-08-04; o login, porém, **não** ocorreu: o pré-requisito só se completa com o `CLI-SETUP-LOGIN-EXEC`. Sob a política aprovada, este bloco deve estar pronto para executar logo após o login, dentro das 8 horas absolutas da credencial.
