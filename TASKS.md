...
**Decisão humana sobre o limite temporal da persistência da credencial — próximo gate, pendente.**

- O `LOGIN-PREP` foi concluído com parecer **B**. Falta uma única decisão humana antes do `LOGIN-EXEC`: qual é o prazo máximo de persistência da credencial da CLI sem progresso do workflow; se esse prazo dispara revogação obrigatória ou apenas reavaliação; e quem executa a revogação quando o workflow for pausado ou abandonado.
- Motivo: todas as demais credenciais e permissões do `ADMIN-B2A5` têm limite explícito — token de impersonação de ~1 hora, binding IAM de 2 horas, conta de serviço desabilitada e preservada 7 dias. A credencial da CLI é a **única** vinculada apenas ao evento `AUTH-REVOKE`, sem limite de tempo, e a documentação oficial adverte que "Any user with access to your file system can use the stored access credentials created by `gcloud auth login`". Sem prazo declarado, a cláusula "qualquer pausa ou abandono do workflow exige revogação explícita" fica inaplicável. **Nenhum prazo foi inventado.**
- Enquanto a decisão não existir, o `LOGIN-EXEC` **não** está autorizado e nenhum prompt-ready foi produzido.

**`ADMIN-B2A5-INVENTORY-AUTH-CLI-SETUP-LOGIN-PREP` — concluído em 2026-08-04, sem login e sem execução de `gcloud`.**

- Parecer: **B. PRONTO COM DECISÃO HUMANA PENDENTE.** Concluído a partir do commit-base `2278f79f14f07235badd55b577e393d85ec7e72c`, exclusivamente por pesquisa em documentação oficial do Google Cloud, análise de segurança, verificação local somente leitura e atualização documental. Zero `gcloud`, login, navegador OAuth, autenticação, token, ADC, acesso remoto a recursos, IAM, API habilitada, inventário, alteração funcional, staging, commit, push ou EXEC iniciado.
...
- Sequência vigente: `LOGIN-PREP` → commit documental do LOGIN-PREP → **decisão humana sobre o limite temporal da persistência** → `CLI-SETUP-LOGIN-EXEC` → `LOGIN-GOVERNANCE` → `AUTH-PROVISION-EXEC` → `PROVISION-GOVERNANCE` → `ACTIVATION-PREP` → `ACTIVATION-EXEC` → `INVENTORY-EXEC` → `AUTH-REVOKE` → governanças correspondentes → `MIGRATION-PREP` somente se necessário → `FIRESTORE-PREP/EXEC` → `RUNTIME-PREP/EXEC` → `ADMIN-B2B` → `ADMIN-B3`. Nenhuma etapa inicia automaticamente.
...
- Pré-requisito operacional: exige CLI instalada e operador autenticado, usando o caminho absoluto de `gcloud.cmd` e o `CLOUDSDK_CONFIG` isolado. A CLI já está instalada, o isolamento foi comprovado pelo `CLI-SETUP-ISOLATION-REPAIR-EXEC` e o `CLI-SETUP-LOGIN-PREP` foi concluído com parecer **B**, ambos em 2026-08-04; o login, porém, **não** ocorreu: o pré-requisito só se completa após a decisão humana pendente sobre a persistência da credencial e o `CLI-SETUP-LOGIN-EXEC`.
...
- Sequência vigente, atualizada pela conclusão do `CLI-SETUP-LOGIN-PREP`: commit documental do LOGIN-PREP → **decisão humana sobre o limite temporal da persistência da credencial** → `CLI-SETUP-LOGIN-EXEC` → `LOGIN-GOVERNANCE` → `AUTH-PROVISION-EXEC` → `PROVISION-GOVERNANCE` → `ACTIVATION-PREP` → `ACTIVATION-EXEC` → `INVENTORY-EXEC` → `AUTH-REVOKE` → governanças correspondentes → `MIGRATION-PREP` somente se necessário → `FIRESTORE-PREP/EXEC` → `RUNTIME-PREP/EXEC` → `ADMIN-B2B` → `ADMIN-B3`.
...
- APIs: verificação somente leitura. IAM API desabilitada **para** o bloco antes de criar. Service Account Credentials API desabilitada **não impede** o PROVISION, que não usa impersonação; registra-se `disabled` e o gate é herdado pelo `ACTIVATION-PREP`.
