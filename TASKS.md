...
3. **Fase 2 — navegação e estrutura:** o `V7-PREP` foi concluído e a estratégia V7A→V7B→V7C1→V7C2 foi aprovada, usando `js/nav-shared.js` como base única. O próximo bloco é somente o `V7A`; os microblocos de execução não foram iniciados. V6 permanece pendente e deve ser reavaliado depois.
...
- O `V7-PREP` foi concluído em seguida a este checkpoint; o próximo bloco aprovado passou a ser o `V7A`. V6 e B3 permanecem pendentes.
...
1. **V7A — compatibilidade do nav-shared (risco baixo-médio, próximo passo, ainda não iniciado):** preparar `js/nav-shared.js` para a home com exceção de padding desktop para `body.home-page` no CSS injetado; novo token `?v=` nas tags ativas de nav-shared; `index.html` completamente intacto; teste das páginas internas.
2. **V7B — cutover atômico da home (risco alto, não iniciado):** substituir a navegação própria pelo nav-shared, removendo no mesmo commit o chrome estático, o hamburger inline, o modal de busca estático, o overlay estático, a barra de progresso estática, o botão voltar ao topo estático, as tags de `js/home-i18n.js` e `js/home-utilitarios.js` e a tag duplicável de `scroll-animations.js`; manter os módulos aposentáveis fisicamente no disco para rollback; manter `js/home-acessibilidade.js`; alinhar o breakpoint mobile.
...
**Módulos e destinos:** sobrevivem `js/home-eventos.js`, `js/home-experiencias.js` e `js/home-contato.js`; `js/home-i18n.js` e `js/home-utilitarios.js` perdem as tags no V7B e os arquivos no V7C1; `js/home-acessibilidade.js` é mantido e depois reduzido; o hamburger inline sai no V7B. `translations.js` permanece intacto, com `window.applyTranslations` e `translationsApplied` como contratos obrigatórios. VLibras estático da home permanece no V7B (guards do shared impedem segunda instância; consolidação para V7C1 ou follow-up). Busca: `search.js` e `search-index.js` intactos; o modal estático sai no V7B e o injetado vira instância única.

**Regra de cache do V7:** `js/nav-shared.js` está em `NEVER_CACHE` no Service Worker; HTML e navegações não são cacheados pelo SW; `sw.js` e `CACHE_NAME` não devem ser alterados em V7A ou V7B; o V7A usa novo token `?v=` nas tags ativas de nav-shared; páginas legadas e `portal-usuario` ficam fora do bloco; os módulos aposentáveis permanecem no disco durante o V7B para rollback.

**Follow-ups registrados (não misturar ao V7A):** o nav-shared não fecha o drawer automaticamente ao redimensionar para desktop; o skip link Alt+3 não existe no shared; o VLibras estático poderá ser consolidado depois; as duas opções `.lang-option.active` serão corrigidas naturalmente pelo shared no cutover.
...
- V7A — próximo microbloco aprovado (compatibilidade do nav-shared), ainda não iniciado; V7B, V7C1 e V7C2 não iniciados. O V7B permanece o passo de alto risco (cutover atômico da home).
