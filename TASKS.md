...
**`ADMIN-B2A5-INVENTORY-AUTH-CLI-SETUP-LOGIN-PREP` — próximo bloco possível, ainda não iniciado.**
...
- Estado: **não iniciado**; exige autorização humana própria. O contrato herdado do `CLI-SETUP-PREP` continua vigente para o login — `auth login --brief`, conta humana autorizada recebida somente em memória, `CLOUDSDK_CONFIG` isolado apenas no processo, caminho absoluto de `gcloud.cmd`, fluxo padrão de navegador local, e proibição de `gcloud init`, `application-default login`, `--update-adc`, `--cred-file`, chave JSON, impersonação, workload identity, `--access-token-file`, projeto padrão e qualquer `gcloud config set`.
- Sequência imediata: `ISOLATION-REPAIR-GOVERNANCE` → commit documental separado → `CLI-SETUP-LOGIN-PREP` → commit documental do LOGIN-PREP → `CLI-SETUP-LOGIN-EXEC` → `LOGIN-GOVERNANCE` → `AUTH-PROVISION-EXEC`.
...
- Pré-requisito operacional: exige CLI instalada e operador autenticado, usando o caminho absoluto de `gcloud.cmd` e o `CLOUDSDK_CONFIG` isolado. A CLI já está instalada e o isolamento foi comprovado pelo `CLI-SETUP-ISOLATION-REPAIR-EXEC` concluído em 2026-08-04, mas o login **não** ocorreu: o pré-requisito só se completa após o `CLI-SETUP-LOGIN-PREP` e o `CLI-SETUP-LOGIN-EXEC`.
...
- Sequência vigente, atualizada pela conclusão do `CLI-SETUP-ISOLATION-REPAIR-EXEC`: `ISOLATION-REPAIR-GOVERNANCE` → commit documental separado → `CLI-SETUP-LOGIN-PREP` → commit documental do LOGIN-PREP → `CLI-SETUP-LOGIN-EXEC` → `LOGIN-GOVERNANCE` → `AUTH-PROVISION-EXEC` → `PROVISION-GOVERNANCE` → `ACTIVATION-PREP` → `ACTIVATION-EXEC` → `INVENTORY-EXEC` → `AUTH-REVOKE` → governanças correspondentes → `MIGRATION-PREP` somente se necessário → `FIRESTORE-PREP/EXEC` → `RUNTIME-PREP/EXEC` → `ADMIN-B2B` → `ADMIN-B3`.
