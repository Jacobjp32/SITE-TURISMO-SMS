...
R1 foi concluído, validado, commitado, enviado por push e publicado. O próximo módulo da Fase 1 é R2, exclusivamente para o carrossel de experiências; R3 formulário, R4 acessibilidade/utilitários e R5 i18n/fallback inline permanecem posteriores, nesta ordem. R1 não deve ser tratado como R2 iniciado.
...
**Separação aprovada:** R1 extraiu somente a lógica da grade "Acontece em breve" para `js/home-eventos.js`. O carrossel de experiências permaneceu inline e será tratado posteriormente em R2, com `js/home-experiencias.js`. R2 é o próximo microbloco, mas não foi iniciado nesta atualização.
...
**Aprendizado para R2–R5:** todo `import()` relativo deve ser reavaliado quando um bloco inline for transferido para arquivo externo; o caminho passa a ser resolvido pela localização do novo script. Caminhos relativos não devem ser copiados cegamente, e o Network deve ser testado para evitar duplicação de diretórios, como `/js/js/`. R2 permanece como próximo módulo, destinado somente ao carrossel em `js/home-experiencias.js`, sem início automático.
