...
**`ADMIN-B2A5-INVENTORY-AUTH-CLI-SETUP-LOGIN-EXEC` (nova execução) — próximo bloco, habilitado e ainda não iniciado.**

- Habilitado pelo `EXECUTABLE-RECOVERY-PREP-DECISION` de 2026-08-04, que aprovou a **opção A** e elevou o parecer do PREP de **B** para **A. PRONTO PARA NOVO ADMIN-B2A5-INVENTORY-AUTH-CLI-SETUP-LOGIN-EXEC**. Prompt-ready integral e corrigido produzido e direcionado ao GPT-5.6 Codex, Reasoning effort High.
...
- **Gate operacional obrigatório antes de iniciar:** confirmar que `LOGIN-GOVERNANCE`, `AUTH-PROVISION-EXEC`, `PROVISION-GOVERNANCE`, `ACTIVATION-PREP`, `ACTIVATION-EXEC`, `INVENTORY-EXEC` e `AUTH-REVOKE` estão prontos para execução praticamente contínua.
...
- **Estado em 2026-08-04: nenhum prazo em curso.** O `LOGIN-EXEC` parou antes do OAuth sob `gcloudExecutableNotLocated`, portanto `loginCompletedAtUtc` não existe e nem o limite de 60 minutos nem o de 8 horas começaram a correr. Não há credencial, binding, conta de serviço ou ADC a revogar.
