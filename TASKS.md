...
3. **Fase 2 — navegação e estrutura:** o `V7-PREP` e o `V7A` foram concluídos; a estratégia V7A→V7B→V7C1→V7C2 segue aprovada, usando `js/nav-shared.js` como base única. O próximo bloco é somente o `V7B`, de alto risco, ainda não iniciado. V6 permanece pendente e deve ser reavaliado depois.
...
- O `V7-PREP` e o `V7A` foram concluídos em sequência; o próximo bloco aprovado passou a ser o `V7B`, de alto risco. V6 e B3 permanecem pendentes.
...
**Regra de cache do V7:** `js/nav-shared.js` está em `NEVER_CACHE` no Service Worker; HTML e navegações não são cacheados pelo SW; `sw.js` e `CACHE_NAME` permaneceram intactos no V7A e não devem ser alterados em V7B; o V7A usa `?v=site-public-v7a-20260716` nas tags ativas de nav-shared; páginas legadas e `portal-usuario` ficaram fora da renovação; os módulos aposentáveis permanecem no disco durante o V7B para rollback.
...
- V7A — compatibilidade do nav-shared concluída, validada, commitada, enviada por push e publicada; commit funcional `4cd0616cb9d393571946f90c97a753eae16e69c3`, com 15 arquivos, 20 inserções e 14 remoções. O V7B permanece como próximo microbloco de alto risco, ainda não iniciado; V7C1 e V7C2 também não foram iniciados.
