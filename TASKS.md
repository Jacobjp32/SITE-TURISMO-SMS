...
**Atualização operacional de 2026-08-07 — estado atual:** o `AUTH-PROVISION-EXEC` posterior ao enablement criou com sucesso o custom role `adminB2A5InventoryRead` e a service account `admin-b2a5-inventory-reader`, sem binding. Uma leitura da policy própria da conta foi inicialmente interpretada como uma binding; a cadeia parou fail-closed, a credencial daquela etapa foi revogada e uma investigação separada, após `ANOMALY-PREP` e novo `LOGIN-EXEC`, comprovou por REST e Google Cloud CLI independentes: `bindingsPropertyPresent = false`, `bindingsArrayCount = 0`, `materialBindingCount = 0`, `policyShapeUnexpected = false`, `policyReadSemanticMatch = true`, `actualBindingExists = false` e `falsePositiveConfirmed = true`. A classe de falso positivo foi reproduzida e confirmada pela leitura real; sua causa é a semântica de array subexpression do PowerShell sobre propriedade ausente/`$null`, sem preservação do comando efêmero histórico exato. Não existe parser persistido no repositório: `failureLocation = PROMPT_CONTRACT_ONLY`. O custom role está criado, íntegro, com as duas permissões exatas e sem binding; a conta está criada, íntegra, `disabled = false`, com zero chave gerenciada pelo usuário e policy própria sem binding material. O último estado comprovado do projeto continua com zero binding para a service account e zero binding usando o custom role. Nenhuma remediation IAM é necessária. Após a pausa, a configuração isolada já estava em estado zero: zero conta credentialed e ativa, zero ADC, zero projeto persistido, zero impersonação e zero access-token file; diretório padrão ausente; nenhuma revogação adicional foi necessária e não existe deadline ativo.
...
**Novo `ADMIN-B2A5-INVENTORY-AUTH-CLI-SETUP-LOGIN-EXEC` → retomada de `ADMIN-B2A5-INVENTORY-AUTH-PROVISION-GOVERNANCE` — próximos blocos, ainda não iniciados.**

- A credencial da investigação anterior não pode ser reutilizada. O novo login exige operador somente em memória, autorização literal própria, baseline de estado zero e novos prazos; este fechamento não executou nem autorizou login.
- A retomada da `PROVISION-GOVERNANCE` deverá revalidar o custom role e a service account, aplicar o contrato estrutural corrigido à policy própria, comprovar zero bindings materiais e documentar o PROVISION.
- Somente depois da `PROVISION-GOVERNANCE` concluída poderá ser habilitado um futuro `ACTIVATION-PREP`. Não apontar diretamente para `ACTIVATION-PREP`, `ACTIVATION-EXEC` ou inventário enquanto a governança não terminar.
