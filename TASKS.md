...
**Atualização operacional de 2026-08-06:** um novo `ADMIN-B2A5-INVENTORY-AUTH-CLI-SETUP-LOGIN-EXEC` foi concluído com classificação **A. LOGIN ISOLADO CONCLUÍDO, OPERADOR AUTENTICADO E ESTADO COMPROVADO**, seguido pelo `LOGIN-GOVERNANCE` concluído e publicado. A credencial humana está ativa somente no diretório isolado; há uma conta credentialed e uma ativa correspondentes ao operador em memória; não existem ADC, projeto, impersonação, access-token file, custom role, conta de serviço ou binding. A janela temporal iniciada em `2026-08-06T11:59:12.8051248+00:00` permanece em andamento, com limite absoluto invariável em `2026-08-06T19:59:12.8051248+00:00`.
...
- O `LOGIN-EXEC` de 2026-08-06 e o respectivo `LOGIN-GOVERNANCE` estão concluídos; a credencial humana permanece ativa somente na configuração isolada, com a janela temporal em andamento.
- O PROVISION exige preflight temporal, credencial ainda válida, parâmetros obrigatórios somente em memória e autorização humana literal conforme o contrato vigente. Esta governança **não presume nem substitui essa autorização**.
...
- Pré-requisito operacional: exige CLI instalada e operador autenticado, usando o caminho absoluto de `gcloud.cmd` e o `CLOUDSDK_CONFIG` isolado. A CLI 578.0.0 está instalada, o isolamento foi comprovado, e o novo `CLI-SETUP-LOGIN-EXEC` e seu `LOGIN-GOVERNANCE` foram concluídos em 2026-08-06. A credencial permanece ativa sob a política de 60 minutos sem progresso material e limite absoluto invariável de 8 horas; o PROVISION ainda exige gate e autorização humanos próprios.
...
- Sequência vigente após o `LOGIN-GOVERNANCE` de 2026-08-06: `AUTH-PROVISION-EXEC` → `PROVISION-GOVERNANCE` → `ACTIVATION-PREP` → `ACTIVATION-EXEC` → `INVENTORY-EXEC` → `AUTH-REVOKE` → governanças correspondentes → `MIGRATION-PREP` somente se necessário → `FIRESTORE-PREP/EXEC` → `RUNTIME-PREP/EXEC` → `ADMIN-B2B` → `ADMIN-B3`.
