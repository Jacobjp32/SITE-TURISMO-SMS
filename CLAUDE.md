...
### Admin/CMS/Firebase — estado após ADMIN-B2A1-EXEC
...
- A ordem obrigatória da frente Firestore é: `ADMIN-B2A2-BRIDGE` (adaptação mínima do consumidor público de notícias), `ADMIN-B2A3` (proteção de drafts de `noticias`), `ADMIN-B2A4` (proteção de `media_library`) e `ADMIN-B2A5` (decisões humanas e contrato de `ativo`/`moderator`). Nenhum desses blocos foi iniciado.
- O próximo bloco é somente `ADMIN-B2A2-BRIDGE-PREP`, de leitura e análise da query pública real de notícias, sem edição, sem executar a bridge e sem alterar Rules. `ADMIN-B2B` continua posterior e não iniciado. Publicação de Rules permanece bloqueada e reservada exclusivamente ao `ADMIN-B3`, mediante autorização própria.
...
- Admin/CMS/Firebase é a frente ativa. `ADMIN-B1-PREP`, `ADMIN-B1B-PREP`, `ADMIN-B2A-PREP` e `ADMIN-B2A1-EXEC` estão concluídos; o próximo bloco é somente `ADMIN-B2A2-BRIDGE-PREP`, ainda não iniciado.
...
- Não iniciar V7C1, V7C2, V6 ou B3 enquanto a frente pública estiver pausada. Na frente administrativa, não iniciar `ADMIN-B2A2-BRIDGE-PREP` nem qualquer EXEC sem novo escopo explícito; publicação de Rules permanece reservada ao `ADMIN-B3` e exige autorização própria.
