...
### Admin/CMS/Firebase — estado após aprovação do ADMIN-B2A4-PREP
...
- O baseline automatizado tornou reproduzíveis os riscos confirmados e evoluiu, no `ADMIN-B2A3-EXEC`, para 69 testes em 5 suítes: 37 de `noticias`, 10 de `media_library`, 9 de `ativo`/`isAdmin`, 10 de `moderator` e 3 do fallback deny.
...
- O `ADMIN-B2A4-PREP` foi concluído e aprovado exclusivamente como análise somente de leitura, com parecer **A. pronto para ADMIN-B2A4-EXEC**. O PREP não alterou arquivo, Rule, teste, runtime ou dado; não executou npm, Emulator, acesso remoto, commit, push, deploy ou publicação; e não iniciou o EXEC nem qualquer etapa posterior.
...
- O contrato atual de `media_library` é `allow read: if true;` e `allow write: if isAdmin();`, permitindo get, list e queries públicas, inclusive enumeração de URLs, nomes, paths e metadados administrativos. A alternativa aprovada é leitura e escrita exclusivamente por Admin: no futuro EXEC, `allow read: if isAdmin();` e `allow write: if isAdmin();`. A escrita permanece exatamente como já está; nenhum helper, campo de publicação/status, schema ou ajuste de `ativo`/`moderator` será incluído.
- A futura proteção preserva get, list integral, leitura de registros legados/esparsos, get inexistente, create, update e delete para Admin ativo. Anônimo, usuário comum, usuário autenticado sem perfil, `moderator` e Admin inativo permanecem sem leitura ou escrita conforme o contrato atual de `isAdmin`; as decisões institucionais de `ativo` e `moderator` continuam reservadas ao `ADMIN-B2A5`.
- O futuro `ADMIN-B2A4-EXEC` fica restrito a `firestore.rules` e `tests/firestore.rules.test.mjs`. Partirá do baseline de 69 testes em 5 suítes, renomeará a suíte e os 10 testes dedicados de `media_library`, inverterá exatamente quatro resultados de leitura, preservará seis resultados, adicionará exatamente 18 testes, removerá zero e deverá terminar em 87 testes em 5 suítes: 37 de `noticias`, 28 de `media_library`, 9 de `ativo`/`isAdmin`, 10 de `moderator` e 3 do fallback deny.
- Os testes futuros usarão somente fixtures sintéticas, incluindo registro legado/esparso, registro compatível com o runtime e identidades de Admin ativo/inativo, `moderator`, usuário comum e usuário autenticado sem perfil. Devem preservar `withSecurityRulesDisabled()` somente para seeds, `clearFirestore()`, cleanup, `node:test`, `assertSucceeds`, `assertFails`, concorrência 1 e o projeto obrigatório `demo-turismo-sms-rules-test`.
- `media_library` é catálogo/metadado no Firestore; `cms-media` contém arquivos físicos e URLs de download no Cloud Storage. Proteger a coleção não revoga URLs existentes, não torna arquivos públicos privados, não altera imagens copiadas nem modifica `storage.rules`. O tratamento de `cms-media` e Storage permanece reservado ao `ADMIN-B2B`; nenhuma alteração de runtime, índice, App Check, CSP ou metadata é necessária no `ADMIN-B2A4`.
- A ordem posterior permanece `ADMIN-B2A4-EXEC` → `ADMIN-B2A5` → `ADMIN-B2B` → `ADMIN-B3`. O EXEC permanece pendente e não iniciado, depende de autorização humana explícita, não poderá publicar Rules e não autoriza etapa posterior. A publicação continua exclusiva do `ADMIN-B3`.
...
- Admin/CMS/Firebase é a frente ativa. `ADMIN-B2A3-PREP`, `ADMIN-B2A3-EXEC` e `ADMIN-B2A4-PREP` estão concluídos; o B2A4-PREP foi aprovado somente por leitura com parecer **A. pronto para ADMIN-B2A4-EXEC**. O próximo bloco possível é exclusivamente `ADMIN-B2A4-EXEC`, ainda não iniciado, restrito futuramente a `firestore.rules` e `tests/firestore.rules.test.mjs`, sem publicação e dependente de autorização humana explícita.
...
- Não iniciar V7C1, V7C2, V6 ou B3 enquanto a frente pública estiver pausada. Na frente administrativa, `ADMIN-B2A3-PREP` e `ADMIN-B2A3-EXEC` estão concluídos; `ADMIN-B2A4` permanece posterior e não iniciado, com PREP e autorização próprios. A publicação de Rules permanece reservada ao `ADMIN-B3` e exige autorização específica.
