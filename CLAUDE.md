...
- Próximos caminhos possíveis: V3 navegação com paridade entre nav da home e `nav-shared`, ajustes de links para `/sabores` e `/onde-ficar`, atalhos mobile e sem unificar a home com `nav-shared` ainda; V4 limpeza de peso morto somente depois e com confirmação item a item; V5 consolidação de eventos; V6 reordenação da home; V7 unificação da navegação como alto risco e para depois; B3 mídia/performance por último.
