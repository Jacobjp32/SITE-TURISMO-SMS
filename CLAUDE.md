...
- A ordem posterior permanece `ADMIN-B2A5` → `ADMIN-B2B` → `ADMIN-B3`. O `ADMIN-B2A5` é o próximo bloco possível, mas não foi iniciado e exige PREP e autorização humana próprios para tratar separadamente `ativo` e `moderator`. O `ADMIN-B2B` e o `ADMIN-B3` também não foram iniciados; a publicação continua exclusiva do `ADMIN-B3`.
...
- Admin/CMS/Firebase é a frente ativa. `ADMIN-B2A3-PREP`, `ADMIN-B2A3-EXEC`, `ADMIN-B2A4-PREP` e `ADMIN-B2A4-EXEC` estão concluídos; o EXEC foi validado funcionalmente em 87/87, commitado e enviado para `origin/main`, sem publicação. O próximo bloco possível é exclusivamente `ADMIN-B2A5`, ainda não iniciado e dependente de PREP e autorização humana próprios para as decisões de `ativo` e `moderator`.
...
- Não iniciar V7C1, V7C2, V6 ou B3 enquanto a frente pública estiver pausada. Na frente administrativa, `ADMIN-B2A3-PREP`, `ADMIN-B2A3-EXEC`, `ADMIN-B2A4-PREP` e `ADMIN-B2A4-EXEC` estão concluídos; `ADMIN-B2A5` é o próximo bloco possível, permanece não iniciado e exige PREP e autorização próprios. A publicação de Rules permanece reservada ao `ADMIN-B3` e exige autorização específica.
