...
- Próximos caminhos possíveis: tratar App Check/reCAPTCHA em localhost como ambiente/debug token; investigar Service Worker interceptando tile do OpenStreetMap em bloco separado se persistir em produção; revisar dados do Firestore para eventos aprovados com `establishmentName` sem `establishmentId` seguro; B4b opcional para migrar Firebase compat de mapa/eventos para import modular sob demanda, somente com teste manual dedicado; B3 mídia/performance por último.
