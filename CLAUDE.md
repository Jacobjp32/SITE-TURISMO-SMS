...
- Próximos caminhos possíveis: V4D fallback inline de traduções permanece pendente, risco médio, só deve ser executado por decisão consciente; alternativa aceitável é manter como dívida técnica documentada. V5B fica pendente para despriorizar recorrentes na grade "Acontece em breve", com risco baixo-médio e exigência de teste visual e funcional da home. V5C fica pendente para higiene editorial de Eventos & Notícias, com risco médio e decisão humana necessária sobre notícias e links reais. V5D fica pendente para revisão anti-envelhecimento de Festas em Destaque, com risco médio e dependência de alteração em `translations.js`, portanto somente com decisão consciente. Depois: V6 reordenação da metade inferior da home; V7 unificação da navegação como alto risco e para depois; B3 mídia/performance por último. Follow-ups separados: revisar CSS órfão `.agrosamas-banner`; revisar chaves i18n órfãs `agrosamas-banner-*`; registrar que `CONFIG.agrosamas` está temporariamente sem efeito na home; revisar CSS órfão `.map-modal-*` em bloco próprio; revisar futuramente as chaves i18n `modal-endereco`, `modal-telefone` e `modal-horario` sem alterar `translations.js` agora; investigar Service Worker em localhost se voltar a interceptar Leaflet/OSM, sem tratar como regressão do V3.
