...
- Admin/CMS/Firebase é a frente ativa. `ADMIN-B2A3-PREP`, `ADMIN-B2A3-EXEC`, `ADMIN-B2A4-PREP`, `ADMIN-B2A4-EXEC` e `ADMIN-B2A5-PREP` estão concluídos; as decisões humanas posteriores ao B2A5-PREP também estão registradas. O EXEC monolítico do B2A5 está bloqueado com classificação **C** e foi substituído por blocos independentes ainda não iniciados e dependentes de autorização própria.
