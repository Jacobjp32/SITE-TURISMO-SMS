...
### Admin/CMS/Firebase — estado após ADMIN-B2A2-CSP-FIX-PROD-VALIDATION
...
- O baseline automatizado tornou reproduzíveis os riscos já confirmados: **P0** — leitura pública de documentos draft em `noticias`; **P1** — leitura pública de `media_library`. Também registrou o contrato atual de `ativo != false`, o comportamento de `moderator` e o fallback global deny, sem aceitar os riscos como decisão institucional.
...
- O `ADMIN-B2A3-PREP` foi concluído e aprovado exclusivamente como análise de leitura, sem alteração funcional, teste, Emulator, acesso remoto, publicação, commit ou push. O `ADMIN-B2A3-EXEC` permanece pendente, não iniciado e dependente de autorização humana explícita posterior.
- O contrato aprovado de leitura de `noticias` usa `publicado == true` booleano como único critério público: anônimo, usuário comum autenticado e `moderator` leem somente documentos com esse valor; ausência, `null`, `false`, string, número, array, objeto ou apenas `status: "publicado"` negam acesso público. `status` não participa da autorização, portanto `publicado: true` com `status: "rascunho"` continua público.
- Admin continuará lendo todos os documentos, inclusive drafts, documentos sem `publicado` e registros legados, por `isAdmin()`. `moderator` não receberá acesso administrativo a drafts neste bloco; seu contrato definitivo permanece reservado ao `ADMIN-B2A5`.
- A Rule escolhida para o futuro EXEC é `allow read: if isAdmin() || (resource.data.publicado == true);`, mantendo `allow write: if isAdmin();`, sem helper novo, `isModerator()`, uso de `status` ou mudança na validação de escrita. Não há `match` sobreposto que conceda leitura alternativa e o fallback recursivo continua deny.
- A ordem da frente Firestore permanece `ADMIN-B2A3-EXEC` → `ADMIN-B2A4` → `ADMIN-B2A5` → `ADMIN-B2B` → `ADMIN-B3`. No futuro `ADMIN-B2A3-EXEC`, somente `firestore.rules` e `tests/firestore.rules.test.mjs` estarão autorizados; qualquer terceiro arquivo exigirá nova autorização. Site público, Admin, Portal, App Check, CSP, dados e runtime não serão alterados.
- A publicação das Firestore Rules continua reservada exclusivamente ao `ADMIN-B3`. O `ADMIN-B2A3-EXEC` não publica Rules e não pode ser iniciado sem novo escopo e autorização explícita.
...
- Admin/CMS/Firebase é a frente ativa. Os blocos até `ADMIN-B2A3-PREP` estão concluídos e aprovados; o PREP foi somente leitura e não alterou funcionalidade. O próximo bloco possível é `ADMIN-B2A3-EXEC`, ainda não iniciado e dependente de autorização explícita posterior.
...
- Não iniciar V7C1, V7C2, V6 ou B3 enquanto a frente pública estiver pausada. Na frente administrativa, o `ADMIN-B2A3-PREP` está concluído e aprovado, mas o `ADMIN-B2A3-EXEC` permanece não iniciado e exige novo escopo e autorização explícita; publicação de Rules permanece reservada ao `ADMIN-B3` e exige autorização própria.
