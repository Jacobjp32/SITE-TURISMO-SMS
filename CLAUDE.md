...
- R4A é o próximo microbloco da Fase 1, ainda não iniciado e restrito à acessibilidade eMAG: fonte, contraste, restauração via `localStorage`, `prefers-reduced-motion`/vídeo e atalhos Alt+1..4. A futura implementação deverá preservar explicitamente `window.changeFontSize` e `window.toggleContrast`, pois quatro atributos `onclick` da home dependem dessas funções globais. R5 i18n/fallback inline permanece posterior e por último.
