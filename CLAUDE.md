...
### Admin/CMS/Firebase — estado após ADMIN-B2A2-NETWORK-DIAG-PREP
...
- O `ADMIN-B2A2-BRIDGE-PREP` e o `ADMIN-B2A2-BRIDGE` foram concluídos. O commit funcional `4b1b783398fa659ebbff7302cdf1038e6bdd184a` está em `origin/main` e alterou `js/cms.js`, `noticias.html`, `noticia.html` e a metadata em `js/site-meta.js`: a consulta pública usa `where('publicado', '==', true)`, resultado vazio do Firestore permanece sucesso com `CMS.source = 'firebase'`, e fallback local ocorre somente em falha.
- O `ADMIN-B2A2-NETWORK-DIAG-PREP` foi encerrado com defeito de CSP confirmado: `https://www.google.com/recaptcha/api.js`, originado por `firebase-app-check.js`, foi bloqueado por `script-src-elem`; como não há diretiva separada, o navegador aplicou `script-src` como fallback, que não autoriza o recurso.
- A matriz confirmou timeout no Firefox em rede residencial, institucional e hotspot móvel, além de timeout no Chrome em hotspot móvel. O bloqueio de CSP/reCAPTCHA ocorreu em todas as combinações; não houve `permission-denied` nem ausência de `CONFIG.firebase`. Portanto, a CSP incompatível é defeito confirmado, mas não está comprovada como causa única do timeout do Firestore.
- Bloqueios de `firebase-app.js.map`, `firebase-firestore.js.map` e `firebase-app-check.js.map` por `connect-src` são recursos de depuração do DevTools e não comprovam, isoladamente, falha de execução dos módulos Firebase.
...
- A ordem da frente Firestore permanece `ADMIN-B2A2-BRIDGE` → `ADMIN-B2A3` → `ADMIN-B2A4` → `ADMIN-B2A5`; somente o bridge foi concluído. `ADMIN-B2A3`, `ADMIN-B2A4`, `ADMIN-B2A5` e `ADMIN-B2B` permanecem não iniciados, e a publicação de Rules continua reservada exclusivamente ao `ADMIN-B3`.
- O próximo bloco recomendado é somente `ADMIN-B2A2-CSP-FIX-PREP`, ainda não iniciado: localizar a fonte exata da meta CSP, propor a alteração mínima e revisar `script-src`/`script-src-elem`, `frame-src` e `connect-src` conforme os requisitos oficiais do reCAPTCHA, preservando todas as demais diretivas. Não alterar `js/cms.js`, Firestore Rules, não iniciar `ADMIN-B2A3` e não testar long polling sem novo bloco explícito.
...
- Admin/CMS/Firebase é a frente ativa. `ADMIN-B1-PREP`, `ADMIN-B1B-PREP`, `ADMIN-B2A-PREP`, `ADMIN-B2A1-EXEC`, `ADMIN-B2A2-BRIDGE-PREP`, `ADMIN-B2A2-BRIDGE` e `ADMIN-B2A2-NETWORK-DIAG-PREP` estão concluídos; o próximo bloco recomendado é somente `ADMIN-B2A2-CSP-FIX-PREP`, ainda não iniciado.
...
- Não iniciar V7C1, V7C2, V6 ou B3 enquanto a frente pública estiver pausada. Na frente administrativa, não iniciar `ADMIN-B2A2-CSP-FIX-PREP`, `ADMIN-B2A3` nem qualquer EXEC sem novo escopo explícito; publicação de Rules permanece reservada ao `ADMIN-B3` e exige autorização própria.
