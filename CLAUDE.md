...
- A ordem da frente Firestore permanece `ADMIN-B2A2-BRIDGE` → `ADMIN-B2A3` → `ADMIN-B2A4` → `ADMIN-B2A5`. O gate para iniciar futuramente o `ADMIN-B2A3-PREP` está liberado, mas o PREP ainda não foi iniciado; `ADMIN-B2A3-EXEC`, `ADMIN-B2A4`, `ADMIN-B2A5` e `ADMIN-B2B` permanecem não iniciados.
- O próximo bloco recomendado é somente `ADMIN-B2A3-PREP`, exclusivamente em leitura e planejamento da proteção de drafts em `firestore.rules`, sem alterar Rules, testes, Emulator ou produção. A publicação de Rules continua reservada exclusivamente ao `ADMIN-B3`.
...
- Admin/CMS/Firebase é a frente ativa. Os blocos até `ADMIN-B2A2-CSP-FIX-PROD-VALIDATION` estão concluídos; o bridge e a correção CSP estão publicados e aprovados. O próximo bloco recomendado é somente `ADMIN-B2A3-PREP`, ainda não iniciado.
...
- Não iniciar V7C1, V7C2, V6 ou B3 enquanto a frente pública estiver pausada. Na frente administrativa, o gate do `ADMIN-B2A3-PREP` está liberado, mas nenhum PREP ou EXEC deve ser iniciado sem novo escopo explícito; publicação de Rules permanece reservada ao `ADMIN-B3` e exige autorização própria.
