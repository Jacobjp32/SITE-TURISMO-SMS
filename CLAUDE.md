...
- R5A também foi concluído como microbloco separado: o fallback inline obsoleto foi removido de `index.html`, aproximadamente 174 linhas foram eliminadas e a declaração `var translations = window.translations || {};` foi preservada. `translations.js`, a cobertura completa PT/EN/ES/PL, o runtime inline do seletor, `sms-lang`, `window.applyTranslations`, `translationsApplied` e a persistência após reload foram preservados e validados. O commit funcional é `55615cd0d0c25db647d9ed0d04decca8e0ea7eb9 refactor(home): remove dicionario fallback inline obsoleto de traducoes`, presente em `origin/main`; V4D foi absorvido e concluído por este microbloco. R1, R2, R3, R4A e R4B permanecem intactos. R5B — externalização do runtime i18n — é o próximo microbloco, ainda não iniciado, e deverá usar o escopo do R5-PREP e carregamento sem `defer`, depois do hamburger e antes de `js/home-acessibilidade.js`.
