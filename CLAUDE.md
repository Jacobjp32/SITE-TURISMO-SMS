...
### Retomada Admin/CMS/Firebase — checkpoint de 2026-07-20

- O `ADMIN-RESTART-PREP` foi concluído somente em leitura, diagnóstico, testes estáticos e smoke sem autenticação. Nenhum arquivo foi alterado, criado ou excluído durante o PREP; não houve escrita em Firestore, Firebase Storage, Firebase Authentication, rules, CORS, dados ou produção.
- Não foram executados commit, push, deploy, seed, migração ou inventário remoto. O working tree encontrado estava em `main`, com a referência local alinhada a `origin/main`, nenhuma alteração rastreada e `.claude/settings.local.json` não rastreado e intocado. A consulta remota independente do `origin` não foi confirmada por falta de credencial no ambiente.
- O painel possui runtime administrativo real, incluindo autenticação, dashboard, aprovações, vínculos, usuários, eventos, notícias, Biblioteca de Mídia, banners, empreendimentos e contratos/fluxos de mídia. A fundação modular em passthrough cobre Dashboard, Banners, Empreendimentos, Context, UI, Registry, Router e Shell; deve evoluir progressivamente, sem reversão ou reescrita ampla, mas não é a prioridade imediata.
- Tratar sempre como sistemas separados: **site público**, **Painel Admin/CMS** e **Portal do Usuário**. Não misturar refatoração, correção ou execução entre eles sem bloco e autorização específicos.
- Permanecem pausados: site público, V7C1, V7C2, V6, B3, otimização de mídia pública, integração CMS → site público e tarefas preparadas para Claude Fable. O Claude Fable não será usado na frente administrativa.
- O próximo bloco único é `ADMIN-B1-PREP`: validação autenticada e anônima, somente leitura, do contrato real de Auth, roles, App Check, Firestore Rules, Storage Rules e CORS. Senha e token nunca devem ser fornecidos ao Codex; o login é manual pelo usuário no navegador. Nenhuma correção, criação, atualização, exclusão, upload, publicação de rules ou aplicação de CORS pertence ao B1.
- Antes de considerar o painel concluído, resolver os bloqueios P0/P1 e cumprir os critérios de painel utilizável registrados em `TASKS.md`. Não assumir que rules locais correspondem integralmente à produção sem evidência remota atual.
- A tag recomendada é `pre-admin-restart-20260720`, mas só poderá ser criada depois da revisão desta governança, do commit documental e do push desse commit. Não criar a tag durante esta atualização.
...
- Admin/CMS/Firebase é a frente ativa. O próximo bloco é somente `ADMIN-B1-PREP`, ainda não iniciado.
...
- Não iniciar V7C1, V7C2, V6 ou B3 enquanto a frente pública estiver pausada. Na frente administrativa, não ultrapassar `ADMIN-B1-PREP` sem novo escopo explícito, evidência do B1 e autorização própria.
