...
- Pendência externa do Formspree: o endpoint permanece `xpqykpqd`, e o Workflow atual continua temporariamente entregando para `imprensapmsms@gmail.com`. O endereço institucional obrigatório `turismo@saomateusdosul.pr.gov.br` já foi adicionado em Linked Emails, mas permanece `PENDING` e depende de confirmação por outro setor. Nenhum envio real deve ocorrer enquanto estiver `PENDING`. Após `VERIFIED`, deve-se selecionar esse endereço em Forms > TURISMO > Workflow > Email, salvar com a ação Enabled, realizar um único envio institucional controlado, confirmar o recebimento no novo endereço e confirmar que o Gmail antigo deixou de receber. A troca ocorrerá somente no painel do Formspree e não exigirá código, metadata, commit ou deploy.
  - Próximos caminhos possíveis: a Fase 1 foi concluída com R1, R2, R3, R4B, R4A, R5A e R5B; o `V7-PREP` foi concluído e a estratégia V7A→V7B→V7C1→V7C2 foi aprovada com cinco decisões humanas. O próximo passo aprovado é somente o `V7A`, ainda não iniciado; V7B, V7C1 e V7C2 não foram iniciados. V6 e B3 permanecem pendentes; o V7 segue sendo a unificação da navegação com `js/nav-shared.js` como base única, com aposentamento planejado de `js/home-i18n.js` e `js/home-utilitarios.js`, V6 deve ser reavaliado após a fundação modular e B3 mídia/performance permanece em fase própria e reservado para depois. V5C3 fica pendente para avaliar a extração do `style` inline dos CTAs para classe compartilhada; exige CSS e não deve ser executado automaticamente. O follow-up arquitetural de uma fonte única de notícias para evitar manutenção duplicada entre home e `noticias.html` permanece fora do V5C, aguardando decisão entre JSON, CMS ou outra solução futura e possível retomada do CMS. V5D fica pendente para revisão anti-envelhecimento de Festas em Destaque, com risco médio e dependência de alteração em `translations.js`, portanto somente com decisão consciente. Follow-ups separados: revisar CSS órfão `.agrosamas-banner`; revisar chaves i18n órfãs relacionadas aos blocos removidos, incluindo `agrosamas-banner-*`, `modal-endereco`, `modal-telefone` e `modal-horario`; registrar que `CONFIG.agrosamas` está temporariamente sem efeito na home; revisar CSS órfão `.map-modal-*` em bloco próprio; planejar a virada anual de `eventos-2026.json`; avaliar futuramente a duplicação entre `eventos-2026.json` e `TURISMO_EVENTOS`; revisar editorialmente o destaque do 32º Mês Polonês após 30/08/2026; registrar para V7 o follow-up de duas opções `.lang-option.active` após reload; investigar Service Worker em localhost se voltar a interceptar Leaflet/OSM, sem tratar como regressão do V3. Alterações futuras nos arquivos `js/home-*.js` ou em `translations.js`, atendidos por cache/precache do Service Worker, devem avaliar obrigatoriamente novo token `?v=` ou nova versão de `CACHE_NAME`; o cache não será alterado nesta governança.
