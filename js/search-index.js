(function () {
  ...
  var fixedEntries = [
    ...
    makeEntry("Pontos Turísticos", "Explore", "Atrações e locais de interesse para conhecer em São Mateus do Sul.", "/mapa-turistico.html?grupo=pontos-turisticos", ["atracoes", "pontos", "turisticos", "locais"]),
    makeEntry("Experiências", "Explore", "Vivências culturais, rurais, gastronômicas e náuticas.", "/mapa-turistico.html?grupo=roteiros", ["experiencias", "vivencias", "turismo", "rural", "cultura"]),
    ...
    makeEntry("Capital Polonesa do Paraná", "Sobre", "Título e legado da herança polonesa na cidade.", "/mapa-turistico.html?categoria=Cultura", ["capital polonesa", "polonesa", "imigracao", "cultura"]),
    makeEntry("Terra da Erva-mate", "Sobre", "Importância econômica e cultural da erva-mate para a região.", "/mapa-turistico.html?grupo=roteiros", ["terra da erva-mate", "erva-mate", "economia", "tradicao"]),
  ...
  var pontoEntries = fromCollection(data.pontos, function (item) {
    return makeEntry(item.nome, "Ponto Turístico", item.descricao, item.url || "/mapa-turistico.html?grupo=pontos-turisticos", item.tags || []);
  });
  ...
  var hospedagemEntries = fromCollection(data.hospedagens, function (item) {
    return makeEntry(item.nome, "Planeje sua visita", item.descricao, item.url || "/mapa-turistico.html?categoria=Hospedagem", (item.tags || []).concat([item.categoria || "hospedagem", item.localizacao || ""]));
  });

  var restauranteEntries = fromCollection(data.restaurantes, function (item) {
    return makeEntry(item.nome, "Sabores", item.descricao, item.url || "/mapa-turistico.html?categoria=Gastronomia", (item.tags || []).concat([item.categoria || "restaurantes"]));
  });
